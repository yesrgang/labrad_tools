...
class NameColumn(QtGui.QGroupBox):
    ...
    def populate(self, names):
        ...
        self.layout.addWidget(NameBox(''))#browse and run
        for i, nb in enumerate(self.name_boxes):
            if not i%15:
                self.layout.addWidget(Spacer(sbheight/2, nbwidth))
            self.layout.addWidget(nb)
        self.layout.addWidget(Spacer(sbheight/2, nbwidth))
        self.layout.addWidget(NameBox('')) #add button
        self.layout.addWidget(NameBox('')) #del button
        self.setLayout(self.layout)
...
class SequencerClient(QtGui.QWidget):
    ...
    def initialize(self):
        ...
        self.setLayout(self.layout)
        #self.setStyleSheet('QWidget {background-color: #eeeeee}')
