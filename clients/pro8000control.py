...
update_time = 100 # [ms]
server_name = 'yesr13 PRO8000'

class LDC80xxWidget(QtGui.QGroupBox):
    state_id = 461001
    current_id = 461002
    power_id = 461003
    hasNewCurrent = False
    ...
    def __init__(self, diode_name, reactor, cxn=None):
    ...
    def populateGUI(self):
        ...
        self.current_box.setRange(self.ctlconf['min_current'], self.ctlconf['max_current'])
        self.current_box.setSingleStep(self.ctlconf['step_size'])
        self.current_box.setDecimals(abs(int(np.floor(np.log10(self.ctlconf['step_size'])))))
    ...
    def requestValues(self, c=None):
        server = yield self.cxn.get_server(server_name)
...
class PRO8000Client(QtGui.QWidget):
    ...
    def populateGUI(self):
        self.setWindowTitle(self.server_name + ' Control')
        if self.layout is None:
