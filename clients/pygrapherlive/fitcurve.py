...
#            height, center, sigma, offset = self.fit(self.fitFuncGaussian, [height, center, sigma, offset], newYData, dataX)
            solutions, errors = self.fit(self.fitFunc, params, which_to_fit, newYData, dataX)
...
#         self.parent.solutionsDictionary[dataset, directory, label, self.curveName, str(self.parameterNames), index] = solutions
            ...
            self.parent.errorsDictionary[dataset, directory, index, self.curveName] = errors
...
#            yield self.cxn.data_vault.add_parameter_over_write('Solutions'+'-'+str(index)+'-'+'Gaussian', [height, center, sigma, offset], context = self.context)        
            ...
            yield self.parent.cxn.data_vault.add_parameter_over_write('Errors'+'-'+str(index)+'-'+self.curveName, errors, context = self.parent.context)
        else:
            solutions = params
            errors = None
...
#        modelY = self.fitFuncGaussian(modelX, [height, center, sigma, offset])
    ...
    def fit(self, function, parameters, fit_these, y, x=None):
        solutions = parameters
        errors = [0.0 for p in parameters]
        def f(x, *fit_parameters):
            all_parameters = parameters
            for i, p in zip(fit_these, fit_parameters):
                all_parameters[i] = p
            return function(x, all_parameters)
        params = np.take(parameters, fit_these)
        popt, pcov = curve_fit(f, x, y, params)
        perr = np.sqrt(np.diag(pcov))
        for i, s, e in zip(fit_these, popt, perr):
            solutions[i] = s
            errors[i] = e
