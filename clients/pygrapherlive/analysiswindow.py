...
from fitdoubleexponential import heaviside, FitDoubleExponential
...
class AnalysisWindow(QtGui.QWidget):
    ...
    def __init__(self, parent, ident):
        ...
        self.fitDoubleExponential = FitDoubleExponential(self)
        self.fitCurveDictionary = {
                                   ...
                                   self.fitDoubleExponential.curveName: self.fitDoubleExponential
                                  }
        self.initUI()
...
#        self.setGeometry(300, 300, 500, 300)
        ...
        self.parameterTable.setColumnCount(5)
...
#        from labrad import units as U    
    ...
    def setupParameterTable(self, curveName):
        ...
        self.parameterTable.setHorizontalHeaderLabels(QtCore.QStringList(['Fit','Parameters','Manual','Fitted', 'Errors']))
...
#        self.horizontalHeader = self.parameterTable.horizontalHeader()
        ...
        try:
            ...
            for parameterName, parameterValue in zip(self.fitCurveDictionary[self.curveName].parameterNames, self.fitCurveDictionary[self.curveName].parameterValues):
                ...
                try:
                    ...
                    self.parent.savedAnalysisParameters[self.dataset, self.directory, self.index, self.curveName][2][parameterName] = None # errors
                except:
        ...
        for parameterName in self.fitCurveDictionary[self.curveName].parameterNames:
            ...
            # errors
            item = QtGui.QTableWidgetItem()
            item.setText(str(self.parent.savedAnalysisParameters[self.dataset, self.directory, self.index, self.curveName][2][parameterName]))
            item.setFlags(QtCore.Qt.ItemIsEnabled)
    ...
    def fitCurves(self, parameters = None, drawCurves = False):
        ...
        if (drawCurves == False):
            ...
            for parameterName in self.fitCurveDictionary[self.curveName].parameterNames:
                ...
                self.parent.savedAnalysisParameters[self.dataset, self.directory, self.index, self.curveName][2][parameterName] = self.errorsDictionary[self.dataset, self.directory, self.index, self.curveName][i]
                i += 1
            ...
            for error in self.errorsDictionary[self.dataset, self.directory, self.index, self.curveName]:
                item = QtGui.QTableWidgetItem()
                item.setText(str(error))
                item.setFlags(QtCore.Qt.ItemIsEditable)
                self.parameterTable.setItem(i, 4, item)
                i += 1
            i = 0
