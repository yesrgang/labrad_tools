...
### BEGIN NODE INFO
[info]
name = VISA Device Manager
version = 1.1
description = Manages discovery and lookup of VISA devices
...
class VISADeviceManager(LabradServer):
    """Manages autodetection and identification of VISA devices.

    The device manager listens for "VISA Device Connect" and
    "VISA Device Disconnect" messages coming from VISA bus servers.
    ...
    def initServer(self):
        ...
        connect_func = lambda c, (s, payload): self.visa_device_connect(*payload)
        disconnect_func = lambda c, (s, payload): self.visa_device_disconnect(*payload)
        mgr = self.client.manager
        self._cxn.addListener(connect_func, source=mgr.ID, ID=10)
        self._cxn.addListener(disconnect_func, source=mgr.ID, ID=11)
        yield mgr.subscribe_to_named_message('VISA Device Connect', 10, True)
        yield mgr.subscribe_to_named_message('VISA Device Disconnect', 11, True)

        # do an initial scan of the available devices
        yield self.refreshDeviceLists()
        
    @inlineCallbacks
    def refreshDeviceLists(self):
        """Ask all VISA bus servers for their available devices."""
        servers = [s for n, s in self.client.servers.items()
                     if (('VISA Bus' in n) or ('visa_bus' in n)) and \
        ...
        for name, (success, addrs) in zip(names, resp):
            if not success:
                ...
                for addr in addrs:
                    self.visa_device_connect(name, addr)

    @inlineCallbacks
    def visa_device_connect(self, server, channel):
    ...
    def visa_device_disconnect(self, server, channel):
        """Handle messages when devices connect."""
...
#             print 'got name', name
    ...
    def register_server(self, c, devices, messageID):
        """Register as a server that handles a particular VISA device(s).
    ...
    def serverDisconnected(self, ID, name):
        ...
        for (server, channel) in list(self.knownDevices.keys()):
            if server == name:
                self.visa_device_disconnect(server, channel)
